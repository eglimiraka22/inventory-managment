...
const JobList = (props) => {
	...
	useEffect(() => {
		if (jobsData) {
			//Filtering notes based on selected category
	...
	const jobsFilteredList = filteredJobs.map((jobs, index) => {
		return (
			<JobItem
	...
	return (
		<React.Fragment>
			<main className='w-full h-[85vh] pt-5  '>
				<div className='flex flex-col   border mx-3 px-1 shadow-xl h-full gap-3 rounded-lg'>
					...
					<div className='w-full  justify-between flex p-1'>
						<h3 className='flex-1 flex gap-2 items-center flex-row'>
							...
							<span>Informative piece of text that can be used regarding to this model</span>
						</h3>
					...
					<div className='w-full  flex flex-col justify-center shadow-md '>
						<ul className=' w-full flex flex-col justify-evenly  items-center'>
							<li className='flex w-full   justify-center py-2   gap-1'>
								<p className='w-full md:w-[70%] flex flex-row justify-center font-bold'>
									<span className='flex-1 text-center'>JobSite</span>{' '}
									<span className='flex-1 text-center'>Status</span>
								</p>
							</li>

							{filterValue ? jobsFilteredList : jobsDataLists}
						</ul>
					</div>
				</div>
			</main>
